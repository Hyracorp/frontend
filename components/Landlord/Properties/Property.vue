                                    ...
                                    <div v-for="(item, index) in slotProps.items" :key="index" class="col-span-1">
                                        <div class="flex flex-col sm:flex-row md:flex-row p-4 gap-3 border-t border-gray-200 dark:border-gray-700">
                                            <div class="w-full sm:w-1/4 md:w-1/5 relative">
                                                <img class="block mx-auto rounded-md w-full" :src="item.first_photo_url" :alt="item.title" />
                                            ...
                                            <div class="flex flex-col sm:flex-row md:flex-row justify-between sm:items-center md:items-start flex-1 gap-4">
                                                <div class="flex flex-col md:flex-row justify-between items-start gap-2">
                                                    <div >
                                                        ...
                                                        <div class="text-lg font-medium text-surface-700 dark:text-surface-0/80 mt-2">{{ item.title }}</div>
                                                     <div class="text-lg font-medium text-surface-700 dark:text-surface-0/80" :class="item.approved==true?'text-green-500':'text-red-500'">
{{item.approved==true?'Approved':'Not Approved'}}
  ...
  <span class="font-medium text-secondary text-sm">{{item.city}},{{item.state}}</span>
                                                ...
                                                <div class="flex flex-col sm:items-end gap-5">
                                                    <span class="text-xl font-semibold text-surface-700 dark:text-surface-0/80">₹{{ item.expected_rate_rent }}/Month</span>
