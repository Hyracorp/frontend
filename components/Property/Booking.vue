...
const bookingActions = computed(() => {
  ...
  const actions = {
    "Pending": {
      ...
      description: "Your booking is currently pending. You can cancel it if you wish."
    },
    "No Show": {
      action: goDashboard,
      buttonText: "Go to Dashboard",
      title: "You missed your appointment",
      description: "You did not attend the appointment. Please contact us for further assistance."
    },
    "Approved": {
      action: cancelBooking,
      buttonText: "Cancel Booking",
      title: "Your booking is approved",
      description: "Your booking has been approved. You can still cancel it if necessary."
    },
    "Cancelled": {
      action: goDashboard,
      buttonText: "Go to Dashboard",
      title: "Your booking was cancelled",
      description: "Your booking has been cancelled. You can schedule a new one from your dashboard."
    },
    "Rejected": {
      action: contactSupport,
      buttonText: "Contact Support",
      title: "Your booking was rejected",
      description: "Unfortunately, your booking was rejected. Please contact support for assistance."
    },
    "Visited": {
      action: goDashboard,
      buttonText: "Go to Dashboard",
      title: "You have visited",
      description: "Thank you for visiting. You can view your details on the dashboard."
    },
    "Finalized": {
      action: goDashboard,
      buttonText: "Go to Dashboard",
      title: "Your booking is finalized",
      description: "Your booking has been finalized. You can check the details on your dashboard."
    },
    "Expired": {
      action: ()=>visible.value = true,
      buttonText: "Book Again",
      title: "Your booking has expired",
      description: "Your booking has expired. Please contact support for rescheduling."
    },
    "Rescheduled": {
      action: goDashboard,
      buttonText: "Go to Dashboard",
      title: "Your booking was rescheduled",
      description: "Your booking has been rescheduled. Please check your dashboard for new details."
    }
  };
...
async function updateProperty() {
 let property = propertyAPI.getProperty(route.params.id)
...
const formattedDate = getFormattedDate(selectedDate.value);
  ...
  let slots = await propertyAPI.getAvailableSlots(property.value.id, formattedDate);

 availableSlots.value = await propertyStore.getAvailableTimeSlots(slots);
        }else{
          toast.add({ severity: 'error', summary: 'Error', detail: 'Please fill required fields', life: 3000 });
        }
...
<template #footer>
<div v-if="bookingActions">
<Button @click="bookingActions.action">{{ bookingActions.buttonText }}</Button>
</div>
</template>
</Card>
</div>
